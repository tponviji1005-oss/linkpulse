...
async function main() {
  ...
  try {
    ...
    // 2. Login
    const login = await request('POST', '/api/auth/login', { email: 'test@linkpulse.com', password: 'Test1234!' });
    const token = login.body.token;
    check('Login returns token', login.status === 200 && !!token);

    // 3. Get links
    const links = await request('GET', '/api/links?page=1&limit=5', null, token);
    ...
    if (hasLink) {
      ...
      // 4. Overview
      const ov = await request('GET', `/api/analytics/${linkId}/overview?period=all`, null, token);
      ...
      // 5. Timeline
      const tl = await request('GET', `/api/analytics/${linkId}/timeline?period=7d`, null, token);
      check('Timeline endpoint', tl.status === 200);
      check('Timeline is array', Array.isArray(tl.body));

      // 6. Devices
      const dev = await request('GET', `/api/analytics/${linkId}/devices?period=all`, null, token);
      ...
      // 7. Browsers
      const br = await request('GET', `/api/analytics/${linkId}/browsers?period=all`, null, token);
      check('Browsers endpoint', br.status === 200);
      check('Browsers is array', Array.isArray(br.body));

      // 8. OS
      const os = await request('GET', `/api/analytics/${linkId}/os?period=all`, null, token);
      check('OS endpoint', os.status === 200);
      check('OS is array', Array.isArray(os.body));

      // 9. Referrers
      const ref = await request('GET', `/api/analytics/${linkId}/referrers?period=all`, null, token);
      ...
      const adv = await request('GET', `/api/analytics/${linkId}?period=all`, null, token);
      ...
      const bad = await request('GET', '/api/analytics/00000000-0000-0000-0000-000000000000/overview', null, token);
      check('Non-owned link 404', bad.status === 404);

      // 12. Unauthorized returns 401
      const noAuth = await request('GET', `/api/analytics/${linkId}/overview`, null, null);
      check('No auth returns 401', noAuth.status === 401);

      // 13. Timeline with period=all
      const tlAll = await request('GET', `/api/analytics/${linkId}/timeline?period=all`, null, token);
